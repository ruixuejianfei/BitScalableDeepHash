...
void caffe_vRngUniform<double>(const int n, double* r, const double a,
		const double b) {
	VSL_CHECK(
			vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, Caffe::vsl_stream(), n, r, a, b));
}
...
void caffe_vRngGaussian<float>(const int n, float* r, const float a,
		const float sigma) {
	VSL_CHECK(
			vsRngGaussian(VSL_RNG_METHOD_GAUSSIAN_BOXMULLER, Caffe::vsl_stream(), n, r, a, sigma));
}
...
void caffe_vRngGaussian<double>(const int n, double* r, const double a,
		const double sigma) {
	VSL_CHECK(
			vdRngGaussian(VSL_RNG_METHOD_GAUSSIAN_BOXMULLER, Caffe::vsl_stream(), n, r, a, sigma));
}
