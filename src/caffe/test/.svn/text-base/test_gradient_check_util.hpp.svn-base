...
public:
	GradientChecker(const Dtype stepsize, const Dtype threshold,
			...
			stepsize_(stepsize), threshold_(threshold), seed_(seed), kink_(
					kink), kink_range_(kink_range) {
	}
...
template<typename Dtype>
void GradientChecker<Dtype>::CheckGradientNet(Net<Dtype>& net,
		vector<Blob<Dtype>*>& input) {
	...
	for (int i = 0; i < layers.size(); ++i) {
		...
		CheckGradientExhaustive(*(layers[i].get()), bottom_vecs[i],
				top_vecs[i]);
	}
}
...
Dtype GradientChecker<Dtype>::GetObjAndGradient(vector<Blob<Dtype>*>& top,
	...
	if (top_id < 0) {
		...
		for (int i = 0; i < top.size(); ++i) {
			...
			memcpy(top_blob_diff, top_blob_data,
					sizeof(Dtype) * top_blob->count());
		}
		loss /= 2.;
	} else {
