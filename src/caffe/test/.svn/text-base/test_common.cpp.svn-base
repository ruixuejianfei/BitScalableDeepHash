...
TEST_F(CommonTest, TestRandSeedCPU) {
	...
	viRngBernoulli(VSL_RNG_METHOD_BERNOULLI_ICDF, Caffe::vsl_stream(), 10,
			(int*) data_a.mutable_cpu_data(), 0.5);
	Caffe::set_random_seed(1701);
	viRngBernoulli(VSL_RNG_METHOD_BERNOULLI_ICDF, Caffe::vsl_stream(), 10,
			(int*) data_b.mutable_cpu_data(), 0.5);
	for (int i = 0; i < 10; ++i) {
...
TEST_F(CommonTest, TestRandSeedGPU) {
	...
	Caffe::set_random_seed(1701);
	CURAND_CHECK(
			curandGenerate(Caffe::curand_generator(),
