...
template<typename Dtype>
__global__ void kernel_get_max(const int num, const int dim, const Dtype* data,
		Dtype* out) {
...
void SoftmaxLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	int dim = bottom[0]->count() / bottom[0]->num();
	CUDA_CHECK(
			cudaMemcpy(top_data, bottom_data,
...
// subtraction
caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, dim, 1, -1., scale_data,
		sum_multiplier_.cpu_data(), 1., bottom_diff);
