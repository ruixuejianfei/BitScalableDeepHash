	...
	for (int n = 0; n < NUM_; ++n) {
		for (int c = 0; c < CHANNEL_; ++c) {
			for (int h = 0; h < HEIGHT_IN_; ++h) {
				...
				memcpy(
						top_data
								+ ((n * CHANNEL_ + c) * HEIGHT_OUT_ + h + PAD_)
										* WIDTH_OUT_ + PAD_,
						bottom_data
								+ ((n * CHANNEL_ + c) * HEIGHT_IN_ + h)
										* WIDTH_IN_, sizeof(Dtype) * WIDTH_IN_);
			}
		}
	}
}
...
__global__ void PaddingForward(const int count, const Dtype* in, Dtype* out,
		const int num, const int channel, const int height_in,
		const int width_in, const int pad) {
...
__global__ void PaddingBackward(const int count, const Dtype* in, Dtype* out,
	...
	if (index < count) {
		...
		int c = index % channel;
		index /= channel;
		out[((index * channel + c) * height_in + h) * width_in + w] = in[((index
				* channel + c) * height_out + h + pad) * width_out + pad + w];
	}
}
